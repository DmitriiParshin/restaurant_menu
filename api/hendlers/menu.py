...
from api.schemas import MenuSchemaBase, MenuSchemaOut
...
def create(menu: MenuSchemaBase,
           db: Session = Depends(get_db)) -> MenuSchemaOut:
...
@router.patch('/{id}')
def update(id: UUID, menu: MenuSchemaBase,
           db: Session = Depends(get_db)) -> MenuSchemaOut:
